...
export default function SyncAllButton() {
  ...
  async function sincronizar() {
    ...
    try {
      ...
      setEstado('ok')
      setMsg(`${data.sincronizadas}/${data.total} sincronizadas${data.erros ? ` · ${data.erros} erro(s)` : ''}`)
      router.refresh()
    } catch {
      setEstado('erro')
      setMsg('Erro de rede.')
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        {estado === 'loading' ? 'Sincronizando…' : '↻ Sincronizar tudo com Asaas'}
      </button>
      {msg && <span className={`text-sm ${estado === 'erro' ? 'text-red-300' : 'text-[var(--text-muted)]'}`}>{msg}</span>}
    </div>
  )
}
